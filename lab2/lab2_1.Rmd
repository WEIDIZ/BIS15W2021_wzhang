...
```{r}
top_speed <- c(veyron, chiron, venom_GT)
mean(top_speed)
```
...
R cannot calculate mean with NA. Means Nas need to be aviod?

4. Recalculate the mean using the following code chunk. Why is the useful?  
It could remove NAs when we need to do some calculations with NAs.
```{r}
